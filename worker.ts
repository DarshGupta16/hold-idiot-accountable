...
import { EventType, asPublic } from "@/lib/backend/types";
...
async function checkHeartbeat() {
  try {
    ...
    const heartbeatVar = await convex.query(asPublic(internal.variables.getByKey), { key: "lastHeartbeatAt" });
    ...
    if (diffSeconds > HEARTBEAT_THRESHOLD_SECONDS) {
      ...
      const activeSession = await convex.query(asPublic(internal.studySessions.getActive));
